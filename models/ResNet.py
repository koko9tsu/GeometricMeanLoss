...
class ResNet(nn.Module):
    ...
    def forward(self, x, use_fc=False, cat=False):
        ...
        if cat and not self.training:
            ...
            x2 = self.layer1(x)
            x2_avg = torch.flatten(self.avgpool(x2), 1)
            x3 = self.layer2(x2)
            x3_avg = torch.flatten(self.avgpool(x3), 1)
            x4 = self.layer3(x3)
            x4_avg = torch.flatten(self.avgpool(x4), 1)
            x5 = self.layer4(x4)

            x5_avg = torch.flatten(self.avgpool(x5), 1)
